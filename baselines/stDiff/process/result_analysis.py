...
def clustering_metrics(adata, gene_seq_test, target, pred, mode="AMI"):
    """
...
    """
    if(mode=="AMI"):
        amis = []
        for genes in gene_seq_test:
            print("Check this out: adata.obs[target, genes] ", adata.obs[target, genes].shape)
            print("Check this out: adata.obs[pred, genes] ", adata.obs[pred, genes].shape)
            ami = adjusted_mutual_info_score(adata.obs[target, genes], adata.obs[pred, genes])
            print("AMI ",ami)
            amis.append(ami)
        ...
        # aris = []
        # for genes in gene_seq_test:
        print("Check this out: adata.obs[target] ", adata.obs[target].shape)
        print("Check this out: adata.obs[pred] ", adata.obs[pred].shape)
