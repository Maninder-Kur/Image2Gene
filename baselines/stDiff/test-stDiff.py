...
parser.add_argument("--step", type=int, default=1500) #1500
parser.add_argument("--epoch", type=int, default=900) #900
...
sp_idx_train = np.load(f"/home/puneet/maninder/baselines/stDiff/process/{args.filename}_train_indices.npy")
sp_idx_test  = np.load(f"/home/puneet/maninder/baselines/stDiff/process/{args.filename}_test_indices.npy")


outdir1 = os.path.join("Result", args.document, "stDiff")
...
sc_data_test = pd.DataFrame(data=adata_seq_test.X, columns=sp_genes)

print(f"sc_data_test ::: {sc_data_test.shape}")

sp_idx_test = np.array(sp_idx_test).flatten()
...
def diffusion_impute():
    ...
    all_pred_res_1 = np.zeros_like(sc_data_test)
    fold_metrics = {}
    for train_ind, test_ind in kf.split(raw_shared_gene):
        ...
        gene_ids_test = np.array(gene_ids_test).flatten() 
        print(f"+++++++++++++++++++ type of imputation :: {type(imputation)}")

        all_pred_res_1[:, gene_ids_test] = imputation[sp_idx_test, :][:,  gene_ids_test]
        y_pred = imputation[sp_idx_test, :][:,  gene_ids_test]

        print(f"+++++++++++++++++++++++++++ y_pred :: ", y_pred.shape)
        y_true = sc_data_test.iloc[:, gene_ids_test]
        results_val = compute_metrics(np.array(y_true), np.array(y_pred), genes=test_gene)
        results_val = {
            'fold': idx,
            'phase': 'val',
    ...
    # stDiff_result_pd = pd.DataFrame(SpaGE_result, columns=sp_genes)
...
print("diffusion_result_pd: ", diffusion_result_pd.shape)
